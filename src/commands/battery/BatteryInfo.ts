...
export class BatteryInfo extends TezlabClient {
  ...
  async getVehicleInfo(email?: string, password?: string): Promise<TezlabResponse | null> {
    try {
      ...
      try {
        ...
        await logError(
          ...
          'BatteryInfo.getVehicleInfo.parse'
        );
        throw parseError;
      }
    } catch (error: unknown) {
      await logError(error, 'BatteryInfo.getVehicleInfo');
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('Login failed') || error.message.includes('Not authenticated')) {
          await fs.unlink(this.tokenFile).catch(async (unlinkError) => {
            await logError(unlinkError, 'BatteryInfo.getVehicleInfo.unlink');
          });
  ...
  async displayVehicleInfo(email?: string, password?: string): Promise<void> {
    const data = await this.getVehicleInfo(email, password);
    if (!data) return;
