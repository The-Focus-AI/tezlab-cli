  ...
  async displayChargeReport(email?: string, password?: string): Promise<void> {
    try {
      ...
      try {
        ...
        console.log(
          `Remaining Range: ${data.remaining_range.toFixed(1)} ${data.est_max_range_units}`
        );
        ...
        if (data.cabin_overheat_show && data.cabin_overheat_mins_idle > 0) {
          console.log(
            `Cabin Overheat Protection Time: ${data.cabin_overheat_idle_duration_string}`
          );
        }
        ...
        await logError(
          ...
          'ChargeReport.displayChargeReport.parse'
        );
        throw parseError;
      }
    } catch (error: unknown) {
      await logError(error, 'ChargeReport.displayChargeReport');
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('Login failed') || error.message.includes('Not authenticated')) {
          await fs.unlink(this.tokenFile).catch(async (unlinkError) => {
            await logError(unlinkError, 'ChargeReport.displayChargeReport.unlink');
          });
